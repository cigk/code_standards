...
#### 3.7 行长限制
代码中每一行文本的长度都应该不超过 120 个字符。虽然关于此规则存在很多争论，但最终决定仍是以 120 个字符为上限，如果行长超过了 120，我们通常有两种方法来缩减行长。
* 提取一个局部变量或方法（最好）。
* 应用换行将一行换成多行。
##### 3.7.1 换行策略
...
例如：
```java
Picasso.with(context).load("url").into(ivAvatar);
```
我们应该使用如下规则：
```java
...
比如：
```java
loadPicture(context, "url", ivAvatar, "Avatar of the user", clickListener);
```
我们应该使用如下规则：
```java
loadPicture(context,
...
### 6 第三方库规范
别再闭门造车了，用用最新最火的技术吧，安利一波：**[Android 流行框架查速表][Android 流行框架查速表]**
